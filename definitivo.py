...
def normal(ecuaciones, u, v):
    ...
    return sp.simplify(sp.Matrix(sp.Matrix(du_ecuaciones).cross(sp.Matrix(dv_ecuaciones))).normalized())

def normal_pt(ecuaciones, u, v, u0, v0):
    """
    Retorna el vetor normal de una superficie en un punto
    No se hacen comprobaciones de tipo
    ...
    """
    n = normal(ecuaciones, u, v)
    return [sp.N(n[i].subs([[u, u0],[v,v0]])) for i in range(3)]
...
def curvaturaGauss_pt(ecuaciones, u, v, u0, v0):
    ...
    return (e_pt*f_pt - f_pt**2) / (E_pt*G_pt - F_pt**2)
...
print(dirPrinc(ecuaciones2, u, v, 0, 0))