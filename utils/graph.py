...
def sup_param_cond_elipse(sup, u, v, a, b ,fig=None, resolucion=100, titulo='Superficie'):
    ...
    if np.isscalar(Z): Z = np.full_like(u_values, Z)
    if not fig: fig = go.Figure()
    fig.add_trace(go.Surface(x=X, y=Y, z=Z, colorscale='ice', showlegend=True, name=titulo, colorbar=dict(x=-0.1),))
    fig.update_layout(scene=dict(aspectmode='data'))
    return fig
...
def param_desc_pt_uv(sup, u, v, u0, v0, dom_u=sp.Interval(-5,5), dom_v=sp.Interval(-5,5), a=None, b=None, fig=None, 
    ...
    if a!=None and b!=None:
        fig = sup_param_cond_elipse(sup, u, v, a, b, titulo=r'$\vec{\varphi}='+sp.latex(sup, mat_delim='(')+r'$')
    else:
        fig = sup_param(sup, u, v, dom_u, dom_v, titulo=r'$\vec{\varphi}='+sp.latex(sup, mat_delim='(')+r'$')
    ...
    fig = point(punto, fig=fig, titulo=r'$\varphi('+sp.latex(u0)+r','+sp.latex(v0)+r')='+sp.latex(punto, mat_delim='(')+r'$')
    ...
    if dirs_principales:
        fig = vector(punto, resultados['d1_pt'], fig, 'yellow', r'$\vec{d_1}$')
...
def sup_imp(sup, x, y, z, dom_x=sp.Interval(-5,5), dom_y=sp.Interval(-5,5), dom_z=sp.Interval(-5,5),
            fig=None, color=None, titulo='Superficie implicita', resolucion=75):
...
def imp_desc_pt(f, x, y, z, x0, y0, z0, 
                ...
                tangente=True, normal=True,fig=None):
